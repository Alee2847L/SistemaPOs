...
<body class="bg-slate-50 text-slate-800 antialiased min-h-screen flex flex-col">
            ...
            <div class="overflow-x-auto rounded-xl border border-slate-200 flex-grow">
                <table class="w-full text-left border-collapse">
                    <thead class="bg-slate-50 border-b border-slate-200 text-slate-600 uppercase text-[11px] font-semibold">
                        <tr>
                            <th class="py-3.5 px-4">ID Contrato</th>
    ...
    <div class="fixed inset-0 bg-slate-900/50 backdrop-blur-xs z-50 flex items-center justify-center p-4 hidden" id="modalNuevoPrestamo">
        <div class="bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-2xl overflow-hidden flex flex-col">
            ...
            <form id="formNuevoPrestamo" class="p-6 space-y-4 text-sm">
                ...
                <div>
                    <label class="block font-semibold text-xs text-slate-700 mb-1">Concepto o Descripción del Préstamo / Artículos:</label>
                    <input type="text" id="prestamo_descripcion" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-purple-500 transition" placeholder="Ej: Préstamo en efectivo / Financiamiento de Moto Yamaha MT-15" required>
                </div>

                <!-- Monto Total y Prima -->
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block font-semibold text-xs text-slate-700 mb-1">Monto Total del Bien / Préstamo (L.):</label>
                    ...
                    <div>
                        <label class="block font-semibold text-xs text-slate-700 mb-1">Monto de Prima / Abono Inicial (L.):</label>
                ...
                <div class="bg-purple-50 border border-purple-200 p-4 rounded-xl space-y-2">
                    <div class="flex justify-between text-slate-700 text-xs sm:text-sm">
                        <span>Capital Financiar (Contrato):</span>
    ...
    <script>
                ...
                html += `
                    <tr class="border-b border-slate-100 hover:bg-slate-50 transition">
                        ...
                        <td class="py-3 px-4 text-center">
                            <button type="button" class="bg-purple-50 hover:bg-purple-100 text-purple-700 text-xs px-3 py-1.5 rounded-lg font-medium transition inline-flex items-center gap-1" onclick="verDetalleCuotas(${p.id})">
        ...
        // --- BUSCADOR DE CLIENTES PARA PRÉSTAMO ---
        ...
        document.getElementById('formNuevoPrestamo').addEventListener('submit', async function(e) {
            ...
            const payload = {
                ...
                plazo_meses: datosCalculadosPrestamo.numeroCuotas, // Compatibilidad
        ...
        function verDetalleCuotas(contratoId) {
            window.location.href = `detalle_contrato.php?id=${contratoId}`;
        }
