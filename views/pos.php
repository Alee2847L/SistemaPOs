...
if (!in_array('pos', $modulos_permitidos)) {
    header('Content-Type: application/json; charset=utf-8');
    echo json_encode([
        'success' => false, 
...
require_once '../config/conexion.php';<?php
// views/pos.php
// 1. INICIAR LA SESIÓN PRIMERO QUE TODO
if (session_status() === PHP_SESSION_NONE) {
    session_start();
...
<body class="bg-slate-50 text-slate-800 antialiased min-h-screen flex flex-col selection:bg-blue-500 selection:text-white">
    ...
    <script>
        ...
        function seleccionarClientePOS(codigo_bp, rtn_dni, nombre, estadoCliente = 'ACT', limiteCredito = 0) {
            ...
            // vencidas sin pagar no puede financiar una nueva compra hasta ponerse
            // al día (solo puede facturarse de contado).
            if (estadoCliente === 'ACT' && codigo_bp !== 'BP000') {
                const preguntarCredito = () => {
                    setTimeout(() => {
                ...
                fetch(`../api/prestamos.php?accion=verificar_mora&codigo_bp=${encodeURIComponent(codigo_bp)}`)
                    ...
                    .then(resMora => {
                        if (resMora.success && resMora.en_mora) {
                            ...
                            document.getElementById('contenedor_btn_credito').style.display = 'none';
                            setTimeout(() => {
                                alert(`⚠️ El cliente ${nombre} tiene ${resMora.cantidad_cuotas_vencidas} cuota(s) en mora (vencida(s) y sin pagar).\n\nDebe ponerse al día antes de poder financiar una nueva compra. Esta venta solo puede procesarse de CONTADO.`);
                            }, 100);
                        } else {
                            preguntarCredito();
                        }
                    ...
                    .catch(() => {
                        // Si falla la verificación por red, no se bloquea al cajero aquí;
                        // el backend (procesar_venta.php) igual valida la mora al guardar.
                        preguntarCredito();
                    });
            } else {
                tipoModalidadVenta = 'contado';
